...
static const AcpiSdtHeader *acpi_find_sdt(const void *rsdp_void, const char sig4[4])
{
    ...
    if (acpi_checksum8(rsdp, 20) != 0)
        return NULL;

    if (rsdp->Revision >= 2 && rsdp->Length >= sizeof(AcpiRsdp))
    {
        if (acpi_checksum8(rsdp, rsdp->Length) != 0)
            return NULL;
    }
    ...
    if (rsdp->Revision >= 2 && rsdp->XsdtAddress)
    {
        ...
        if (memcmp(xsdt->Signature, "XSDT", 4) != 0)
            xsdt = NULL;
        else if (acpi_checksum8(xsdt, xsdt->Length) != 0)
            xsdt = NULL;
    }
    ...
    if (!xsdt && rsdp->RsdtAddress)
    {
        ...
        if (memcmp(rsdt->Signature, "RSDT", 4) != 0)
            rsdt = NULL;
        else if (acpi_checksum8(rsdt, rsdt->Length) != 0)
            rsdt = NULL;
    }
...
static void serial_discover_from_spcr(BootInfo *boot_info)
{
    ...
    if (!spcr_hdr)
        return;
    if (spcr_hdr->Length < sizeof(AcpiSpcr))
        return;
    ...
    if (spcr->BaseAddress.AddressSpace != 1)
        return;
    if (spcr->BaseAddress.Address == 0)
        return;
...
static int bootinfo_add_serial_port(BootInfo *bi, uint16_t io_base)
{
    ...
    {
        if (bi->serial.ports[i].io_base == io_base)
            return 0;
    }

    if (bi->serial.count >= HOBBYOS_MAX_SERIAL_PORTS)
        return -1;
...
void serial_discover_ports(BootInfo *boot_info)
{
    ...
    for (UINTN i = 0; i < count; i++)
    {
        ...
        if (!(class_sub == 0x00 || class_sub == 0x02))
            continue;
